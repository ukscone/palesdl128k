...
/*** This file contains the emulation code 
  MODED IM0 Interrupt (as per 1999 Kogel ADAM emu                        ***/
/***                                                                      ***/
...
extern unsigned char Z80_In(unsigned int port);
extern void Z80_Out(unsigned int port,unsigned char value);
...
//this is to cut the autorepeat down to size
unsigned int vars_done=0;       //tests whether 6233/4   (5) has been written to - and we can update it with my own reprate

//static unsigned long cyc;
/*
int donmi=0;
int doP80=0;
int P80bit=5;
...
unsigned int cycl_d=0;//noof cycles so far this execution of z80
unsigned int tapecyclesles=0; //measures pulse widths output by port84

//  int trace_on;
//  int trace_handle;
unsigned int ei_addr=0;


extern byte z80ports_in[];
//void update_sound();
//void flush_sound();
static byte tbyt;
static unsigned char sound_running=0;
...
//THESE HAVE CONTINUITY ACROSS RESETS - altered by VB
static int Z80_IPeriod=50000;
static int Z80_ICount=50000;

#ifdef DEBUG
static int Z80_Trace=0;
static int Z80_Trap=-1;
#endif
#ifdef TRACE
...
//static FILE *stream, *stream2;

/*

   int  _declspec(dllexport) start_trace()
{
        int ret;
                trace_on=1;
   // Open for write 
  if( (stream2 = fopen( "TRACEDUMP.TXT", "w+" )) == NULL )
                ret=MessageBox(NULL,"DIDNT OPEN TRACE FILE","PALE Debug - Press Yes to Exit, CANCEL to NoTrap",MB_YESNOCANCEL | MB_DEFBUTTON2);
   //   msgbox("The file 'data2' was not opened");
//          trace_handle = open( "trace",O_WRONLY | O_CREAT | O_TRUNC | O_TEXT);//,_S_IWRITE);

//stream2 = fopen( "data2", "w+" );
return(1);                      

}
   int  _declspec(dllexport) stop_trace()
{
                trace_on=0;
                fclose(stream2);
return(1);                      
...
//Modded by ME - should take PC off HALT forwards - not back onto itself ???
// --R.PC.W.l;
 ++R.PC.W.l;
 R.HALT=1;
...
static int ffr=0;
static int hu=0;
static int done_1stint=0;


//CPM ALMOST !!!
//if(Z80_IRQ==0){Z80_IRQ=1;return;}
//else
//Z80_IRQ=0;
...
if ( !R.IFF1 )//&& hu==0)  //&& !R.IFF2) //added by me to sorta fix cpm without the above cludge && R.IFF2)
{
 ...
 if (j==Z80_NMI_INT || R.IFF1)
 {
  ...
  else
  {
   ...
   if (R.IM==2)
   {
    ...
    if (R.IM==1)
    {
 //MessageBox(NULL,"IRQ M1","PALE ",MB_YESNOCANCEL | MB_DEFBUTTON1);
...
void Z80_RegisterDump (void)
{
 ...
 printf
 (
   "PC:%04X - AF:%04X HL:%04X DE:%04X BC:%04X SP:%04X IX:%04X IY:%04X\n",
   R.PC.W.l,R.AF.W.l,R.HL.W.l,R.DE.W.l,R.BC.W.l,R.SP.W.l,R.IX.W.l,R.IY.W.l
 ); 
 printf ("STACK: ");
...
//HEAVILY MODIFIED - should be elsewhere
//   int     save_mem_dump();

unsigned int Z80_Execute ()
{
        static unsigned opcode;
        static char lbl1[500];
        static char lbl2[300];
        static char lbl3[300];
        static unsigned int curpc;
        static unsigned int sound_cyc=0;
        static int trapc=0;
        static int trapped=0;
        static int input_count=0;
        static unsigned ret;
        static int z80_icount_last=1000;  //Z80_IPeriod;


        Z80_Running=1;
        InitTables ();
 
        z80_icount_last=Z80_IPeriod;
...
void Z80_Retn (void)
{
//from z80 docs on web
//      R.IFF1=R.IFF2;

        //Copied from Maryat Fuzlins Code
//      if(R.IFF1&R.IFF2) R.IFF1|=R.IFF1;
//      else R.IFF1&=~R.IFF1;
//      M_RET;
 } 

void Z80_RegisterDump_String (unsigned char emu_query1[],unsigned char emu_query2[])
{
        int i;
        char lbl1[300],lbl2[300],lbl3[300],lbl4[300];
 
        sprintf((char*)emu_query1,
                "PC:%04X I: %02X AF:%04X HL:%04X DE:%04X BC:%04X SP:%04X IX:%04X IY:%04X\n",
           R.PC.W.l,R.I,R.AF.W.l,R.HL.W.l,R.DE.W.l,R.BC.W.l,R.SP.W.l,R.IX.W.l,R.IY.W.l);
        sprintf (lbl2,"");

        //STACK
         for (i=0;i<10;++i)
        {
                sprintf(lbl3,"%04X ",M_RDMEM_WORD((R.SP.D+i*2)&0xFFFF));
                strcat(lbl2,lbl3);
        }
        strcpy((char*)emu_query2,lbl2);
        //#ifdef TRACE
        // puts ("PC TRACE:");
        // for (i=1;i<=256;++i) printf ("%04X\n",pc_trace[(pc_count-i)&255]);
        //#endif
...
/*
   unsigned int      set_trapthis (unsigned int huj)
{
//      trapthis=huj;
//      return(trapthis);
   ...
   unsigned int      set_trapaddr (unsigned int huj)
{
//      trapaddr=huj;
//      return(trapaddr);
}
...
unsigned int get_ix ()
{
        return (unsigned int)R.IX.W.l;
}


unsigned int get_de ()
{
        return (unsigned int)R.DE.W.l;
}


/*
unsigned int get_tapecycles()
{
//      return (tapecycles);
}

void set_tapecycles(unsigned int ii)
{
//      tapecycles=ii;
}
*/
...
*/
   ...
   void    get_regs (Z80_Regs *i)  //KOGEL
{
//      char lbl[100];

/*
sprintf(lbl," INPUT  DE is  %04X    DE h is %02X  l is %02X",R.DE  ,R.DE.B.h,R.DE.B.l);
MessageBox(NULL,lbl,"PALE ",MB_YESNOCANCEL | MB_DEFBUTTON1);
i.DE=R.DE;      
sprintf(lbl," OUTPT  DE is  %04X    DE h is %02X  l is %02X",i.DE  ,i.DE.B.h,i.DE.B.l);
MessageBox(NULL,lbl,"PALE ",MB_YESNOCANCEL | MB_DEFBUTTON1);
*/
/*
i->AF=R.AF.W.l;         
...
R.R=i.R;        
R.IM=i.IM;      
//IFF1,IFF2,HALT,IM,I,R,R2;

//NOTE R2  and  HALT  not emulated by RAZE core ?
...
{
        if(reg==Z80_REG_PC)return((UWORD)R.PC.W.l);
        if(reg==Z80_REG_AF)return((UWORD)R.AF.W.l);
        if(reg==Z80_REG_HL)return((UWORD)R.HL.W.l);
        if(reg==Z80_REG_DE)return((UWORD)R.DE.W.l);
        if(reg==Z80_REG_BC)return((UWORD)R.BC.W.l);
        if(reg==Z80_REG_SP)return((UWORD)R.SP.W.l);
        if(reg==Z80_REG_IX)return((UWORD)R.IX.W.l);
        if(reg==Z80_REG_IY)return((UWORD)R.IY.W.l);
        if(reg==Z80_REG_IR)return((UWORD)R.I<<8+(UWORD)R.R);
...
{
    if(reg==Z80_REG_PC)R.PC.W.l = value;
    if(reg==Z80_REG_HL)R.HL.W.l = value;
    if(reg==Z80_REG_DE)R.DE.W.l = value;
    if(reg==Z80_REG_IX)R.IX.W.l = value;
    if(reg==Z80_REG_IY)R.IY.W.l = value;
    if(reg==Z80_REG_SP)R.SP.W.l = value;
    if(reg==Z80_REG_AF2)R.AF2.W.l = value;
    if(reg==Z80_REG_BC2)R.BC2.W.l = value;
    if(reg==Z80_REG_DE2)R.DE2.W.l = value;
    if(reg==Z80_REG_HL2)R.HL2.W.l = value;
    if(reg==Z80_REG_IFF1)R.IFF1 = value;
    if(reg==Z80_REG_IFF2)R.IFF2 = value;
    if(reg==Z80_REG_IR){  R.I = value>>8; R.R = value;  }
    if(reg==Z80_REG_IM){  R.IM = value;  }
    
    if(reg==Z80_REG_IR){  R.R2 = R.R;  }



//    if(reg==Z80_REG_IRQVector)R.DE.W.l = value;
  //  if(reg==Z80_REG_IRQLine)R.DE.W.l = value;
...
//fixme - here immediately stop the exec - used in lineblanking
