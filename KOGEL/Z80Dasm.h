...
//extern  int _stdcall Z80_Dasm (char *buffer,char *dest,unsigned int PC)
{
 char *S;
 char *r;
 int i,j,k;
 ...
 for (j=0;S[j];++j)
 {
  switch (S[j])
  {
   ...
   case 'R':
    ...
    break;
   case 'W':
    sprintf ((char *)buf,"$%04x",buffer[i]+buffer[i+1]*256);
    i+=2;
    strcat (dest,(char *)buf);
    break;
   case 'X':
    sprintf ((char *)buf,"(%s%c$%02x)",r,Sign(buffer[i]),Abs(buffer[i]));
    i++;
    strcat (dest,(char *)buf);
    break;
   case 'Y':
    sprintf ((char *)buf,"(%s%c$%02x)",r,Sign(Offset),Abs(Offset));
    strcat (dest,(char *)buf);
    break;
   case 'I':
    strcat (dest,r);
